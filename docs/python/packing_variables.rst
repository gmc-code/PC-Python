...
| The packing operator, /*,  can be used to unpack the variable vals, so that the tuple (0, 5) is turned into 2 separate arguments, 0 and 5, instead of staying as the single tuple (0, 5).

| The code below runs as if the arguments to the range function were 0 and 5.
| The code prints out: 1, 2, 3, 4, 5, 6 

.. code-block:: python
 
    tuple1 = ("a", "b", "c")
    tuple2 = (3, 2, 1)
    meregd_tuple = (*tuple1, *tuple2)
    print(meregd_tuple)
