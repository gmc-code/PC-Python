...
quantity_menu.grid(row=3, column=1)
quantity_var.trace_add("write", update_total_cost)

# Cost display
cost_display_var = tk.StringVar(root)
cost_display_var.set("Cost per pizza: $0")
tk.Label(root, textvariable=cost_display_var).grid(row=4, column=0, columnspan=2)

# Total cost display
total_cost_var = tk.StringVar(root)
total_cost_var.set("Total cost: $0")
tk.Label(root, textvariable=total_cost_var).grid(row=5, column=0, columnspan=2)
...
tk.Label(root, text="Delete Order for Customer:").grid(row=8, column=0)
delete_customer_var = tk.StringVar(root)
delete_customer_menu = tk.OptionMenu(root, delete_customer_var, "")
delete_customer_menu.grid(row=8, column=1)
